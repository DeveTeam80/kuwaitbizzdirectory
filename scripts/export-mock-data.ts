...
async function main() {
    ...
    const admin = await prisma.user.upsert({
        where: { email: 'admin@kuwaitbizzdirectory.com' },
        update: {},
        create: {
            email: 'admin@kuwaitbizzdirectory.com',
    ...
    const owner = await prisma.user.upsert({
        where: { email: 'owner@kuwaitbizzdirectory.com' },
        update: {},
        create: {
            email: 'owner@kuwaitbizzdirectory.com',
    ...
    // Kuwaiti cities for location detection
    const KUWAITI_CITIES = [
    ...
    for (const listing of listData) {
        try {
            ...
            const isGlobal = !KUWAITI_CITIES.includes(listing.city.toLowerCase())

            // Determine location detection method
            const locationDetection = isGlobal ? 'auto' : 'auto'
            const locationVerified = !isGlobal // Auto-verify Kuwaiti cities
            ...
            console.log(`✅ Imported: ${listing.title} (${isGlobal ? '🌍 Global' : '🇰🇼 Kuwait'})`)
            imported++
        } catch (error: any) {
