  ...
  try {
    ...
    if (!category) {
      return { 
        ...
        description: `Browse ${categorySlug.replace(/-/g, ' ')} businesses in Kuwait.` 
      };
    }
    
    return {
      name: category.name,
      description: category.description || `Browse ${category.name} businesses in Kuwait.`
    };
  } catch (error) {
...
function generateOptimizedTitle(
  ...
  const siteName = seoConfig.siteName || 'Kuwait Bizz Directory';
  
  switch (type) {
    case 'listing':
      return `${businessName} in ${location} - ${category} | ${siteName}`;
    case 'category':
      return `Top ${category} in Kuwait | Verified Businesses Directory`;
    case 'location':
      return `${location} Business Directory | Local Companies Kuwait`;
    default:
...
function generateAIAgentData(
  ...
  const entities = [businessName, category, location, 'Kuwait', 'business directory'];
  const topics = [category.toLowerCase(), 'business services', 'local directory', 'Kuwait business'];
  ...
  switch (type) {
    ...
    case 'category':
      ...
      questionAnswer = [
        {
          question: `What ${category} businesses are available in Kuwait?`,
          answer: `Our directory features verified ${category} businesses across Kuwait. Browse our comprehensive listings to find the right service provider for your needs.`
        },
        {
      ...
      conversationalHooks = [
        `Discover top ${category} businesses in Kuwait`,
      ...
      questionAnswer = [
        {
          ...
          answer: `Our directory features various businesses in ${location}, Kuwait including restaurants, services, shops, and more. Browse by category to find what you need.`
        }
      ];
      conversationalHooks = [
        `Explore businesses in ${location}, Kuwait`,
...
function buildOrganizationSchema(
  ...
  return {
    ...
    description: listing.fullDescription?.[0] || listing.desc || `${listing.title} - Professional business services in Kuwait`,
    ...(address && { address }),
    ...(telephone && { telephone }),
    ...
    ...(isLocalBusiness && { areaServed: { '@type': 'Country', name: 'Kuwait' } }),
...
function buildFAQSchema(listing: any): object | null {
  ...
  const location = listing.city || listing.location || 'Kuwait';
  ...
  return {
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: `${listing.title} is located in ${location}, Kuwait. ${
...
export async function generateCategoryPageSEOMetadata(
  ...
  const fallbackTitle = totalItems > 0
    ? `${normalizedName} in Kuwait (${totalItems}+ Listings) | ${siteName}`
    : `${normalizedName} in Kuwait | ${siteName}`;
  const fallbackDescription = `${description} Browse ${totalItems > 0 ? `${totalItems}+ ` : ''}verified ${normalizedName.toLowerCase()} across Kuwait. Updated regularly with new businesses.`;
  ...
  const aiAgentData = generateAIAgentData(normalizedName, normalizedName, 'Kuwait', description, 'category');
  ...
  const jsonLd = [
    ...
    ]),
    {
      ...
      description: `Comprehensive directory of ${normalizedName.toLowerCase()} in Kuwait`,
      itemListOrder: 'http://schema.org/ItemListOrderAscending',
      numberOfItems: totalItems,
      ...(itemListElements.length > 0 && { itemListElement: itemListElements })
    },
    // AI Agent optimization schemas
    ...buildAIAgentSchema(normalizedName, normalizedName, 'Kuwait', description, 'category')
  ];

  return generateSEOMetadata(pathname, {
    title: pageTitle,
    description: pageDescription,
    keywords: `${normalizedName} Kuwait,${normalizedName} directory,Kuwaiti ${normalizedName.toLowerCase()},${normalizedName} companies,best ${normalizedName.toLowerCase()} Kuwait`,
...
export async function generateListingPageSEOMetadata(
  ...
  keywordParts.push('Kuwait', 'business directory', 'verified business');
...
export async function generateBusinessSEOMetadata(
  ...
  const customData: CustomSEOData = {
    ...
    keywords: `${businessName},${businessLocation},${businessCategory},Kuwait business,business directory`,
    canonical: `${seoConfig.siteUrl}${pathname}`,
    businessName,
    businessDescription,
    businessLocation,
...
export async function generateCategorySEOMetadata(
  ...
  const customData: CustomSEOData = {
    title: generateOptimizedTitle(categoryName, categoryName, 'Kuwait', 'category'),
    description: `${categoryDescription} Find verified ${categoryName.toLowerCase()} businesses across Kuwait.`,
    keywords: `${categoryName} Kuwait,${categoryName} directory,Kuwait ${categoryName.toLowerCase()}`,
    canonical: `${seoConfig.siteUrl}${pathname}`,
    ogTitle: `${categoryName} Businesses in Kuwait`,
...
export async function generateLocationSEOMetadata(
  ...
  const customData: CustomSEOData = {
    ...
    description: `${locationDescription} Discover local businesses, services, and companies in ${locationName}, Kuwait.`,
    keywords: `${locationName} businesses,${locationName} directory,companies in ${locationName},${locationName} Kuwait`,
    canonical: `${seoConfig.siteUrl}${pathname}`,
