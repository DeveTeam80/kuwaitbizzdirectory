...
function AdminListingsContent() {
    ...
    const handleVerifyLocation = async (id: number, currentIsGlobal: boolean) => {
        const userChoice = window.confirm(
            `Current location type: ${currentIsGlobal ? 'Global (🌍)' : 'Kuwait (🇰🇪)'}\n\n` +
        ...
        if (!userChoice) { // User clicked Cancel, wants to change
            const changeToGlobal = window.confirm('Change to Global (outside Kuwait)?\n\nOK for Global, Cancel for Kuwait.')
        ...
        try {
            ...
            alert(`Location verified as ${finalIsGlobal ? 'Global 🌍' : 'Kuwait 🇰🇪'}!`)
            fetchListings()
        } catch (err: any) {
            alert(`Error: ${err.message}`)
        } finally {
    ...
    const getLocationTypeBadge = (listing: Listing) => {
        ...
        return <span className="badge" style={{ backgroundColor: '#008450', color: 'white' }}>🇰🇪 Kuwait</span>
    }
